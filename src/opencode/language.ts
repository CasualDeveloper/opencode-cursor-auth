...
function usage(counts?: CursorTokenUsage): LanguageModelV3Usage {
  return {
    inputTokens: {
      total: counts?.input,
      noCache: uncachedInputTokens(counts),
    ...
    outputTokens: {
      total: counts?.output,
      text:
        counts?.output !== undefined && counts.reasoning !== undefined
