  ...
  constructor(domain: Plugin.Context["event"]) {
    this.task = (async () => {
      try {
        ...
        this.failure = new Error("Cursor host tool observation ended");
        for (const session of this.sessions) session.failed(this.failure);
        this.sessions.clear();
        for (const watch of this.watches) watch.failed(this.failure);
        this.watches.clear();
      }
    })();
  }
